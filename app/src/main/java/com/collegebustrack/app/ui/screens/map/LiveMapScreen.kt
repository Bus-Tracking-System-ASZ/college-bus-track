...
fun LiveMapScreen(
    ...
    AppScaffold(AppSection.MAP, onSelect, "Live Map") {
        Column(
            ...
            Row {
                Text("Last updated: 11:45 AM", color = Muted, style = MaterialTheme.typography.labelSmall)
            ...
            Panel {
                ...
                Text("Blue Line", fontWeight = FontWeight.SemiBold)
                Text("Total distance: 18.6 km", color = Muted, style = MaterialTheme.typography.bodySmall)
                HorizontalDivider(Modifier.padding(vertical = 12.dp))
                Text("Stops & ETA", fontWeight = FontWeight.SemiBold)
                data.stops.forEach { stop ->
                    Row(
                        Modifier.fillMaxWidth().padding(top = 9.dp)
                    ) {
                        Text(stop.time, style = MaterialTheme.typography.labelMedium)
                        Spacer(Modifier.width(18.dp))
                        Text(stop.name, modifier = Modifier.weight(1f))
                        Text(if (stop.state == StopState.COMPLETE) "✓" else "•")
                    }
                }
            }
        }
